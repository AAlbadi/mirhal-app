    ...
    try {
        ...
        for (const row of rows) {
            try {
                ...
                // Multi-photo support: Check Photo 1 through Photo 6
                let photos = [];
                for (let i = 1; i <= 6; i++) {
                ...
                if (photos.length === 0) {
                    photos = (row.get('Photos') || '').split(',').map(s => s.trim()).filter(Boolean);
                }
                ...
                // Matching logic remains same, but now it will update the status too
                const tolerance = 0.001;
                const existingSpot = await Vehicle.findOne({
                    title: name,
...
async function exportToGoogleSheets(spreadsheetId, credentials) {
    try {
        ...
        // Define exact headers as per admin_guide
        const headers = [
            'Spot ID', 'Name', 'NameAr', 'Emoji', 'Location', 'City',
